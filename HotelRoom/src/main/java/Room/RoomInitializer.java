        ...
        try {
            ...
            for(int i = 0; i < numFloors; ++i) {
                ...
                for(int j = 0; j < roomsPerFloor; ++j) {
                    ...
                    roomMatrix[i][j] = new RentableRoom(
                            RoomID.buildRoomID(initRoomTheme, initRoomSize,
                                    initQualityLevel, initSmokingStatus, currRoomNumber),
                            RoomData.DEF_ROOM_STAT, currRoomNumber);

                    CentralDatabase.insertIntoHotelRoomsData(roomMatrix[i][j].toString());
