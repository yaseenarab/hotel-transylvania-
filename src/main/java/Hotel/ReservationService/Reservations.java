...
public class Reservations  {
    public static class Marsha {
        private static Integer reservationIDNum;
        private static Map<String, RoomReservation> reservations;
        ...
        public static void addReservation(RoomReservation reservation) {
            reservations.put(reservation.getResID(), reservation);
        }
    ...
    public static class RoomInitializer {
        ...
        public static RoomQuality getRoomAvailable(RoomQuality room) {
            for(int i = 0; i < NUM_FLOORS; ++i) {
                for(int j = ROOM_NUM_MIN; j <= ROOM_NUM_MAX; ++j) {
                    if(floorRooms[i][j].getRoomStatus().equals(RoomStatus.VaCl) &&
                            /*floorRooms[i][j].getRoomType().equals(room.getRoomType()) &&*/
                            floorRooms[i][j].getBedType().equals(room.getBedType()) &&
                            /*floorRooms[i][j].getQualityLevel().equals(room.getQualityLevel()) &&*/
                            floorRooms[i][j].getSmokingStatus().equals(room.getSmokingStatus())) {
                        return floorRooms[i][j];
                    }
                }
            }
            return null;
    ...
    public String makeNewReservation(String profileID, Room room, Date start, Date end) throws Exception {
        try {
            String reservationID = Marsha.generateReservationID();
    ...
    public RoomReservation modifyExistingReservation(String reservationID, Room room, String start, String end) throws Exception {
        try {
            if(reservationExists(reservationID)) {
