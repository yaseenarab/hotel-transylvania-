...
public class RentableRoom extends Room implements RoomData {
    ...
    public RentableRoom(String roomID, RoomStatus roomStatus, Integer roomNumber) throws Exception {
        try {
    ...
    private void setRoomID(String roomID) throws Exception {
        if(roomID == null) {
            ...
            MyLogger.logger.log(Level.SEVERE, "Error in RentableRoom.setRoomID: roomID is empty");
            throw new Exception();
        }
        else if(roomID.length() != ROOM_ID_SIZE) {
            MyLogger.logger.log(Level.SEVERE, "Error in RentableRoom.setRoomID: roomID should be " +
                    ROOM_ID_SIZE + " characters, found " + roomID.length() + ", received " + roomID);
            throw new Exception();
        }
    ...
    private void setRoomFromID(String roomID) throws Exception {
        if(roomID == null) {
            MyLogger.logger.log(Level.SEVERE, "Error in RentableRoom.setRoomFromID: roomID is null");
            throw new Exception();
        }
        else if (roomID.isEmpty()) {
            MyLogger.logger.log(Level.SEVERE, "Error in RentableRoom.setRoomFromID: roomID is empty");
            throw new Exception();
        }
        else if(roomID.length() != ROOM_ID_SIZE) {
            MyLogger.logger.log(Level.SEVERE, "Error in RentableRoom.setRoomFromID: roomID should be " +
                    ROOM_ID_SIZE + " characters, found " + roomID.length() + ", received " + roomID);
            throw new Exception();
        }
        else if(!RoomID.checkValidRoomID(roomID)) {
    ...
    public String toString() {
        return this.roomNumber + "," + this.roomStatus + "," + this.roomTheme + "," +
                this.bedType + "," + this.qualityLevel + "," + this.smokingStatus;
    }
}
